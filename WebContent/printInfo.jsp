...
<script type="text/javascript">
		function getPost(){
			var myselect=document.getElementById("theme");//拿到select对象
			...
			var activeIndex = myselect.options[index].value;//拿到选中项options的value
			 if (activeIndex == "0"){
				alert ("亲，请先选择一个主题吧");
				selectObj.focus();
				return false;
			...
			$.ajax({
				url:"printPost.do",
				data:{"content":content,"title":title,"id":activeIndex},
				type:"post",	
	...
	<body>
		<div id="father">
			...
			<c:if test="${id==0}">
				<b>主题：</b>
				<select id="theme" name="theme" style="width: 117px;height: 32px">
				<option value="0">-----请选择-----</option>
				<c:forEach items="${list }" var="pro">
				<option value="${pro.id }">${pro.tname}</option>
				</c:forEach>
			</select>&nbsp;&nbsp;&nbsp;&nbsp;
			</c:if>
